...
class TemporalGAT(nn.Module):
    def __init__(self, node_features, hidden_channels, num_heads=4, dropout=0.3, use_fp16=True):
        ...
        with autocast(enabled=self.use_fp16):
            ...
            for i in range(num_chunks):
                ...
                if edge_time is not None:
                    ...
                    # FP16 için edge_attr için özel düzenleme
                    with autocast(enabled=False):
                        x_high_precision = x.float()
                        time_weights_high_precision = time_weights.float()
                        chunk_output = checkpoint(
                            lambda x_lambda, ei_lambda, w_lambda: self.gat1(x_lambda, ei_lambda, edge_attr=w_lambda),
                            x_high_precision, chunk_indices, time_weights_high_precision,
                            use_reentrant=False
                        )
                else:
                    ...
                    with autocast(enabled=False):
                        x_high_precision = x.float()
                        dummy_edge_attr_high_precision = dummy_edge_attr.float()
                        chunk_output = checkpoint(
                            lambda x_lambda, ei_lambda, ea_lambda: self.gat1(x_lambda, ei_lambda, edge_attr=ea_lambda),
                            x_high_precision, chunk_indices, dummy_edge_attr_high_precision,
                            use_reentrant=False
                        )
                
                # Add residual connection and normalize
                if i == 0:
                    chunk_output = chunk_output + x_high_precision  # Residual for first chunk
            ...
            for i in range(num_chunks):
                ...
                # Add residual connection and normalize
                if i == 0:
                    chunk_output = chunk_output + x_high_precision
