    ...
    def _process_node_features(self, num_total_nodes):
        ...
        try:
            if os.path.exists(self.node_feature_path):
                ...
                # One-hot encoding yerine embedding kullan
                embedding_dim = 256  # Embedding boyutu (model boyutuyla uyumlu)
                print(f"No node features file found at '{self.node_feature_path}', using embeddings with dimension {embedding_dim} for {num_total_nodes} nodes.")
                
                # Standardize edilmiş rastgele başlangıç değerleri
                # Xavier/Glorot başlatma mantığı kullanılıyor - daha iyi sonuçlar verir
                std = np.sqrt(2.0 / (num_total_nodes + embedding_dim))
                features = torch.randn(num_total_nodes, embedding_dim) * std
                
                # Değerleri normalize et
                features = F.normalize(features, p=2, dim=1)  # Unit norm
                
                return features
        except Exception as e:
            print(f"Warning: Using embeddings with dimension 256 due to error: {e}")
            embedding_dim = 256
            std = np.sqrt(2.0 / (num_total_nodes + embedding_dim))
            features = torch.randn(num_total_nodes, embedding_dim) * std
            features = F.normalize(features, p=2, dim=1)  # Unit norm
