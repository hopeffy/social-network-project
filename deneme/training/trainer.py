...
class TemporalLinkPredictionTrainer:
    ...
    def __init__(self, model, lr=0.001, weight_decay=5e-4, use_fp16=True):
        """
        ...
        for epoch in tqdm(range(num_epochs), desc="Epochs"):
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{num_epochs}", leave=False)):
                try:
                    ...
                    # Scale loss and backprop (with FP16 optimization)
                    if self.use_fp16:
                    ...
                    torch.cuda.empty_cache()  # Boş bellek bloklarını serbest bırak
