...
def load_model(model_path, device):
    # Model configuration - Hata mesajlarından alınan doğru boyutlar
    node_features = 9  # Input feature dimension
    hidden_channels = 256  # 64'ten 256'ya değiştirildi (yüklenen modele uygun)
    num_heads = 4     # 8'den 4'e değiştirildi (yüklenen modele uygun)
    dropout = 0.2     # main.py'de varsayılan değer
    
    print(f"Model configuration: node_features={node_features}, hidden_channels={hidden_channels}, num_heads={num_heads}")
    ...
    try:
        ...
        # Kontrol noktası yapısını kontrol et
        if isinstance(checkpoint, dict):
            print(f"Checkpoint content keys: {list(checkpoint.keys())}")
        else:
            print(f"Checkpoint is not the expected dict type, current type: {type(checkpoint)}")
            if isinstance(checkpoint, torch.nn.Module):
                # Doğrudan model nesnesi yüklendi
        ...
        if 'model_state_dict' in checkpoint:
            ...
            print(f"Current model expects {len(model_state_dict)} parameters")
            ...
            if 'gat1.lin_src.weight' in state_dict and 'gat1.lin.weight' in model_state_dict:
                print("Adapting GAT structure from old version...")
            ...
            for key in model_state_dict.keys():
                if key in state_dict and model_state_dict[key].shape == state_dict[key].shape:
                    ...
                    if key in state_dict:
                        print(f"Size mismatch: {key}, expected {model_state_dict[key].shape}, loaded {state_dict[key].shape}")
                    else:
                        print(f"Missing parameter: {key}")
                    # Model durumundaki varsayılan değerleri koru
                    compatible_state_dict[key] = model_state_dict[key]
            
            # Uyumluluk oranını raporla
            compatibility_percentage = (compatible_params / total_params) * 100
            print(f"Model compatibility ratio: {compatibility_percentage:.2f}% ({compatible_params}/{total_params} parameters)")
            
            if compatibility_percentage < 10:
                print("WARNING: Very low compatibility ratio. Model parameters may be incompatible.")
                print("Model parameter examination:")
                ...
                if len(common_keys) > 0:
                    print("\nExample size mismatches:")
                    for key in list(common_keys)[:5]:  # First 5 examples
                        if model_state_dict[key].shape != state_dict[key].shape:
                            print(f"  {key}: {model_state_dict[key].shape} vs {state_dict[key].shape}")
            ...
            if compatibility_percentage > 90:
                ...
                print("Model partially loaded (some parameters initialized with default values)")
            else:
                print("Model mostly initialized with default values, performance may be poor")
        elif 'state_dict' in checkpoint:
            # Alternatif anahtar ismi
            print("Using 'state_dict' key...")
            state_dict = checkpoint['state_dict']
            
            # Burada benzer bir uyumluluk kontrolü yapılabilir
            model.load_state_dict(state_dict, strict=False)
            print("Model loaded (strict=False)")
        else:
            # Doğrudan durum sözlüğü olabilir
...
def test_model(model, node_features, edge_index, edge_attr, labels, device):
    ...
    # Fixed threshold metrics for 0.35
...
def main():
    ...
    # Model path
    model_path = "model-27m-test-edilecek/model_best_27m.pt"
    
    # Test data path
    test_file = "test-data/input_A_initial.csv"
    
    # Output file path
    output_file = "output_A.csv"
    ...
    try:
        ...
        # Threshold value
        threshold_value = results['fixed_threshold']  # Use fixed threshold from results
