...
def main():
    ...
    if torch.cuda.is_available():
        ...
        # FP16 ayarlarını yazdır
        print(f"FP16 Mixed Precision: {'Enabled' if args.use_fp16 else 'Disabled'}")
        if args.use_fp16:
            print("- FP16 reduced precision reduction: Enabled")
    ...
    # Shared memory sorunlarını önlemek için iş parçacığı sayısını ve bellek optimizasyonlarını ayarla
    num_dataloader_workers = 0  # İş parçacığı kullanmayarak shared memory sorununu çöz
    persistent_workers = False  # İş parçacığı olmadığında persistent işçilere gerek yok
    prefetch_factor = None  # İş parçacığı olmadığı için prefetch faktörü ayarlanmadı

    train_loader = DataLoader(
        train_signal_instances, 
    ...
    ) if train_signal_instances else None
    
    val_loader = DataLoader(
        val_signal_instances, 
        batch_size=args.batch_size, 
        shuffle=False, 
        num_workers=num_dataloader_workers, 
        pin_memory=False, 
        persistent_workers=persistent_workers,
        prefetch_factor=prefetch_factor
    ) if val_signal_instances else None
    
    test_loader = DataLoader(
        test_signal_instances, 
        batch_size=args.batch_size, 
        shuffle=False, 
        num_workers=num_dataloader_workers, 
        pin_memory=False, 
        persistent_workers=persistent_workers,
        prefetch_factor=prefetch_factor
    ) if test_signal_instances else None 
